...
# Added - for internal optimizations
...
when defined(i386) or defined(amd64):
  ...
  func mm_or_ps*(a, b: m128): m128 {.importc: "_mm_or_ps", x86.}
    ## Bit-or each 4 floats

  # Added

  func mm_set_ps*(d, c, b, a: float32): m128 {.importc: "_mm_set_ps", x86.}
    ## Store 4 floats in little endian order
  func mm_rcp_ps*(a: m128): m128 {.importc: "_mm_rcp_ps", x86.}
    ## Reciprocal each 4 floats
  func mm_sqrt_ps*(a: m128): m128 {.importc: "_mm_sqrt_ps", x86.}
    ## Sqrt each 4 floats
  func mm_rsqrt_ps*(a: m128): m128 {.importc: "_mm_rsqrt_ps", x86.}
    ## Reciprocal-sqrt each 4 floats
  func mm_movemask_ps*(a: m128): uint8 {.importc: "_mm_movemask_ps", x86.}
    ## Set each of 4 lesser bits of target mask based on the MSB of each float in the m128
  func mm_cmpeq_ps*(a, b: m128): m128 {.importc: "_mm_cmpeq_ps", x86.}
    ## Compare floats in a, b and check for equality
  func mm_and_ps*(a, b: m128): m128 {.importc: "_mm_and_ps", x86.}
    ## Compute bitwise and of each 4 floats
  func mm_andnot_ps*(a, b: m128): m128 {.importc: "_mm_andnot_ps", x86.}
    ## Compute bitwise (not a) and b of each 4 floats
  func mm_cmplt_ps*(a, b: m128): m128 {.importc: "_mm_cmplt_ps", x86.}
    ## Compute a < b for each of 4 floats and store either 0xFFFFFFFF or 0 in each result
  func mm_xor_ps*(a, b: m128): m128 {.importc: "_mm_xor_ps", x86.}
    ## Compute bitwise a xor b for each of 4 floats
  func mm_unpackhi_ps*(a, b: m128): m128 {.importc: "_mm_unpackhi_ps", x86.}
    ## Unpack and interleave floats from high halves of a and b.
  func mm_unpacklo_ps*(a, b: m128): m128 {.importc: "_mm_unpacklo_ps", x86.}
    ## Unpack and interleave floats from low halves of a and b.
  func mm_shuffle_ps*(a, b: m128, imm8: uint8): m128 {.importc: "_mm_shuffle_ps", x86.}
  ...
  # Added
  func mm_store_ss*(mem_addr: ptr float32, a: m128) {.importc: "_mm_store_ss", x86.}
  ## Store the lowest element of the m128 in the (unaligned) address.
  func mm_set_ss*(val: float32): m128 {.importc: "_mm_set_ss", x86.}
  ## Return a m128 with lowest element set, others zero.
  func mm_mul_ss*(a, b: m128): m128 {.importc: "_mm_mul_ss", x86.}
  ## Store product of lowest f32 in lowest of dst, then copy upper 3 from a to dst
  func mm_sub_ss*(a, b: m128): m128 {.importc: "_mm_sub_ss", x86.}
  ## Store difference of lowest f32 in lowest of dst, then copy upper 3 from a to dst
  ...
  # Added
  func mm_dp_ps*(a, b: m128, imm8: uint8): m128 {.importc: "_mm_dp_ps", x86.}
    ## Conditionally multiply the packed floats in a and b using the
    ## high 4 bits in imm8, sum the four products, and conditionally
    ## store the sum in dst using the low 4 bits of imm8.

  func mm_blend_ps*(a, b: m128, imm8: uint8): m128 {.importc: "_mm_blend_ps", x86.}
    ## Blend packed floats from a and b using control mask imm8
